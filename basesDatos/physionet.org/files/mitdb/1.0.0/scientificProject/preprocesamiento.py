...
#
#
#     aux=transformer.transform(diccionarioAnotacionesencoder[fic].values)
#
#     columns_to_keep = aux[:, [6, 7]]
#     aux = np.concatenate((columns_to_keep, aux[:, :6], aux[:, 8:]), axis=1)
#
#     newpandas=pd.DataFrame(aux)
#     newpandas.iloc[:,2:-3].to_csv("./window_encoder_resumido/"+fic+".csv", index=False, header=False)
#
...
numeros_posibles=sorted(list(diccionarioDatos.keys()))#[2:3]
